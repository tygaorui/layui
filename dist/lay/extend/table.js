...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'linq'], function (exports) {
  "use strict";
  ...
  var $ = layui.$
    ...
    , table = {
      config: {
        ...
        , depthName: 'LAY_TREE_DEPTH' //tree 深度
        , spreadName: 'LAY_TREE_SPREAD'
      } //全局配置项
      , cache: {} //数据缓存
      , rowTempCache: {} //内容行模板缓存
  ...
  //生成内容行模板
  Class.prototype.RowTemp = function () {
    var that = this
      , options = that.config
      , temp = table.tempCache[that.key];
    if (temp) return temp;

    var trTemp = '<tr data-index="{{ d.' + table.config.indexName + ' }}" data-id="{{ d.' + TREE_ID + ' }}" data-parentid="{{ d.' + TREE_PARENTID + ' }}">';
    temp = {
      tr: [trTemp],
      trs_fixed: [trTemp],
      trs_fixed_r: [trTemp]
    ...
    that.eachCols(function (i, col) {
      var isTree = col.tree_id !== undefined && col.tree_parentid !== undefined;
      if (isTree) {
        options.tree_id = col.tree_id;
        options.tree_parentid = col.tree_parentid;
      }

      var field = col.field || i;

      var td = ['<td data-field="' + field + '"'];
      if (col.edit) td.push(' data-edit="' + col.edit + '"');
      if (col.align) td.push(' align="' + col.align + '"');
      if (col.toolbar) td.push(' data-off="true"');
      if (col.event) td.push(' lay-event="' + col.event + '"');
      if (col.style) td.push(' style="' + col.style + '"');
      if (col.minWidth) td.push(' data-minwidth="' + col.minWidth + '"');
      td.push('>');
      td.push('<div class="layui-table-cell laytable-cell-' + options.index + '-' + field);
      if (col.type !== 'normal')
        td.push(' laytable-cell-' + col.type);
      td.push('">');

      if (isTree) {
        td.push('{{new Array(d.' + table.config.depthName + '+1).join(\'<span class="layui-tree-indent"></span>\') }}');
        td.push('<span class="layui-icon layui-tree-spread" name="layTableArrow">&#xe{{ d.' + table.config.spreadName + '?"625":"623"}};</span>');
        if (col.field) {
          td.push('<span class="layui-tree-text">{{d.' + col.field + '}}</span>');
        }
      }
      else if (col.type === 'numbers')
        td.push('{{d.' + table.config.indexName + '+1}}');

      else if (col.toolbar)
        td.push($(col.toolbar).html() || '');
      else if (col.field)
        td.push('{{ d.' + col.field + ' }}');
      ...
      var tdTemp = td.join('');
      temp.tr.push(tdTemp);
      if (col.fixed && col.fixed !== 'right') temp.tr_fixed.push(tdTemp);
      if (col.fixed === 'right') temp.tr_fixed_r.push(tdTemp);
    });

    temp.tr.push('</tr>');
    temp.tr_fixed.push('</tr>');
    temp.tr_fixed_r.push('</tr>');

    temp.tr = temp.tr.join('');
    temp.tr_fixed = temp.tr_fixed.join('');
    temp.tr_fixed_r = temp.tr_fixed_r.join('');
  ...
  //递归获得Tree结构数据
  Class.prototype.treeData = function (data, parentId) {
    var recursion = function (rows, treeRows, pid, depth) {

      var items = [];
      $.each(rows, function (i, item) {
        if (item[TREE_PARENTID] == pid) {
          item[table.config.depthName] = depth;
          items.push(item);
        }
      });

      parent == pid ? depth = 0 : depth++;
      ...
      if (items.length > 0) {
        if (treeRows.length > 0) {
          treeRows[treeRows.length - 1][table.config.spreadName] = true; //TODO 默认展开还是折叠。
        }
        $.each(items, function (_, item) {
          treeRows.push(item);
          recursion(rows, treeRows, item[TREE_ID], depth);
        });
      }
    }

    var treeData = [];
    recursion(data, treeData, parentId, 0);
    return treeData;
  }
  ...
  Class.prototype.renderData = function (res, curr, count, sort) {
    var that = this
      ...
      , trs_fixed = []
      , trs_fixed_r = []
      , rootid = options.rootid === undefined ? '0' : options.rootid

      //渲染视图
      , render = function () { //后续性能提升的重点

        data = that.treeData(data, rootid);

        table.cache[that.key] = data;

        var trTemp = that.rowTempCache();

        layui.each(data, function (i, item) {
          item[table.config.indexName] = i;

          trs.push(laytpl(trTemp.tr).render(item));
          trs_fixed.push(laytpl(trTemp.tr_fixed).render(item));
          trs_fixed_r.push(laytpl(trTemp.tr_fixed_r).render(item));
        });
        ...
        layui.each(data, function (i1, item1) {
          ...
          if (!sort) {
            ...
            item1[table.config.spreadName] = (options.spread === undefined || options.spread) ? true : false;
          }
          ...
          that.eachCols(function (i3, item3) {
            ...
            var td = ['<td data-field="' + field + '" ' + function () {
              ...
              , '<div class="layui-table-cell laytable-cell-' + function () { //返回对应的CSS类标识
                ...
                var isTree = item3['tree_id'] != undefined && item3['tree_parentid'] != undefined;

                //渲染复选框列视图
                if (!isTree && item3.type === 'checkbox') {
                ...
                } else if (!isTree && item3.type === 'numbers') { //渲染序号
                  return numbers;
                }
                ...
                if (isTree) {
                  ...
                  var ti = new Array(item1[table.config.depthName] + 1).join('<span class="layui-tree-indent"></span>');
                  if (data[i1 + 1] && data[i1 + 1][TREE_PARENTID] == item1[TREE_ID])
                    ti += '<span class="layui-icon layui-tree-spread" name="layTableArrow">' + icon.arrow[item1[table.config.spreadName] ? 1 : 0] + '</span>';
                  else
                    ti += '<span class="layui-tree-indent"></span>';

                  if (item3.type == 'checkbox') {
                    ti += '<div class="layui-tree-checkbox"><input type="checkbox" name="layTableCheckbox" lay-skin="primary"' + function () {
                      var checkName = table.config.checkName;
                      //如果是全选
                      if (item3[checkName]) {
                        item1[checkName] = item3[checkName];
                  ...
                  if (item3.templet)
                    item3 = '<span class="layui-tree-text">' + item3.templet + '</span>';
                  else
                    content = '<span class="layui-tree-text">' + content + '</span>';

                  return ti + (item3.templet ? laytpl($(item3.templet).html() || String(content)).render(tplData) : content);
                }
          ...
          var trsattr = 'data-index="' + i1 + '" data-id="' + item1[TREE_ID] + '" data-parentid="' + item1[TREE_PARENTID] + '"';
          trs.push('<tr ' + trsattr + '>' + tds.join('') + '</tr>');
          trs_fixed.push('<tr ' + trsattr + '>' + tds_fixed.join('') + '</tr>');
          trs_fixed_r.push('<tr ' + trsattr + '>' + tds_fixed_r.join('') + '</tr>');
        });
        */
  ...
  table.clearCacheKey = function (data) {
    ...
    delete data[table.config.depthName];
    delete data[table.config.spreadName];
    return data;
  };
